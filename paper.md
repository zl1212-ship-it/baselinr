...
- [`hedges_g()`](https://zl1212-ship-it.github.io/baselinr/reference/hedges_g.md)
  ...
  comparison group for a continuous covariate, using the pooled
...
- [`cox_index()`](https://zl1212-ship-it.github.io/baselinr/reference/cox_index.md)
  computes the WWC Cox index for a binary covariate,
  $`d_{Cox} = (\mathrm{logit}(p_t) - \mathrm{logit}(p_c)) / 1.65`$,
  placing a difference in proportions on a scale comparable to Hedges’ g
  \[@wwc2022\].
- [`wwc_classify()`](https://zl1212-ship-it.github.io/baselinr/reference/wwc_classify.md)
  maps standardized effect sizes to the three WWC categories:
  `satisfied` ($`|es| \le 0.05`$), `satisfied_with_adjustment`
  ($`0.05 < |es| \le 0.25`$), and `not_satisfied` ($`|es| > 0.25`$)
  \[@wwc2022\].
